...
public class TopCaTest {
    ...
    public void shouldExtracTopCA() throws ParseException, Exception {
		...
		TopCAJ7Extractor.getTop100CAJ7(f.getParent(),
				f.getParent(),
				new SimpleDateFormat("yyyyMMdd").parse("20190629"));
		File expected  = new File(classLoader.getResource("top_ca_expected.data").getFile());
		File generated  = new File(classLoader.getResource("top_100_ca_6776b618-bf03-450c-9400-cdf78e768062_20190629_J7.data").getFile());
		
		System.out.println(FileUtils.readFileToString(expected, "utf-8").trim().replace("\n","")
				);
		System.out.println(FileUtils.readFileToString(generated, "utf-8").trim().replace("\n","")
				);
		assertEquals("error!", 
